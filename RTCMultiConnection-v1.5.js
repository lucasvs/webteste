...
/*
// startRecording && stopRecording audio/video sync implemented.
                ...
                var mediaConfig = {
                    onsuccess: function(stream, returnBack, idInstance) {
                        ...
                        if (self.onspeaking) {
						  ...
						  if (self.onaudionly) {
                            var audioMeter = new AudioMeter({
                                context: self._audioContext,
                                root: self,
                                event: streamedObject
                            });
                            audioMeter.connectToSource(stream);
                        }
                    },
                    onerror: function() {
                        var error;
                ...
                if (response.suggestRenegotiation) {
                    ...
                    if (isData(renegotiate))
                        ...
                        root.captureUserMedia(function(stream00) {
                            if(stream00) peer.connection.addStream(stream00);
                            createOffer();
                        }, renegotiate);
    ...
    function AudioMeter(config) {
        ...
        this.script.onaudioprocess = function(event) {
            ...
            var volume = that.volume.toFixed(2);
             ...
             root.onaudionly(volume);
       
        };
    }

	  AudioMeter.prototype.connectToSource = function(stream) {
